// API Route for Veeam Backup for Microsoft 365 Protected Users
import { NextResponse } from 'next/server';
import { getVB365Config, refreshVB365Token } from '@/lib/server/vb365-helper';
import { createLogger } from '@/lib/logger';
const logger = createLogger('VBM/ProtectedUsers');
...
export async function GET(request: Request) {
    try {
        ...
        const endpoint = `/v8/ProtectedUsers?${searchParams.toString()}`;
        ...
        return NextResponse.json({ error: 'Failed to fetch VBM users' }, { status: 500 });
    }
}
