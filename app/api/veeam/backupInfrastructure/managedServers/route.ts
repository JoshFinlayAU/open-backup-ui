// API Route for Veeam Managed Servers
...
const logger = createLogger('VBR/ManagedServers');
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: `Failed to fetch managed servers: ${response.status} - ${errorText}` },
        ...
        return NextResponse.json(
            { error: error instanceof Error ? error.message : 'Failed to fetch managed servers' },
            { status: 500 }
        );
    }
}
